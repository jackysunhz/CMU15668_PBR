...
public:
  ...
  virtual bool isSSS() const {return false;}
  virtual bool trackSpectral() const {return false;}
};
...
class SpectralDielectric : public Material
{
public:
	SpectralDielectric(const json & j = json::object());
	...
	bool trackSpectral() const override {return true;}

	float ior;      ///< The (relative) index of refraction of the material
};

class SpectralDiffuse : public Material
{
public:
	SpectralDiffuse(const json & j = json::object());
...
{
	MicrofacetDielectric(const json & j = json::object());
