...
SpectralDielectric::SpectralDielectric(const json & j)
{
	ior = j.value("ior", ior);
}

bool SpectralDielectric::scatter(const Ray3f &ray, const HitInfo &hit, const Vec2f &sample, Color3f &attenuation, Ray3f &scattered) const
{
	Vec3f normal;
	float eta1, eta2;

	float lambda = (float)((ray.lambdaSample * 300.f) + 400.f)/1000.f;
	float real_ior = ior + 0.2f/(lambda*lambda);
	// ensure ior and normal are correctly oriented for computing reflection and refraction
	//cout << "IOR: " << real_ior << endl;
...
bool SpectralDielectric::sample(const Vec3f & dirIn, const HitInfo &hit, const Vec2f &sample, ScatterRecord &srec) const 
{
	Vec3f normal;
	float eta1, eta2;
...
Color3f SpectralDielectric::eval(const Vec3f & dirIn, const Vec3f & scattered, const HitInfo & hit) const 
{
  return Color3f(0.f);
}

float SpectralDielectric::pdf(const Vec3f & dirIn, const Vec3f & scattered, const HitInfo & hit) const 
{
  return 1.f;
}

// SpectralDiffuse
SpectralDiffuse::SpectralDiffuse(const json & j)
{
	albedo = parseTexture(j.at("albedo"));
}

bool SpectralDiffuse::scatter(const Ray3f &ray, const HitInfo &hit, const Vec2f &sample, Color3f &attenuation, Ray3f &scattered) const
{
	scattered = Ray3f(hit.p, hit.sn + randomOnUnitSphere(sample));
	int lambda = int(ray.lambdaSample * 300) + 400;
	Color3f rgbAlbedo = albedo->value(hit);
	attenuation = albedo->value(hit);
	return true;
}

bool SpectralDiffuse::sample(const Vec3f & dirIn, const HitInfo &hit, const Vec2f &sample, ScatterRecord &srec) const 
{
	ONBf onb;
	onb.build_from_w(hit.sn);
    srec.scattered = onb.toWorld(randomCosineHemisphere(sample));
...
Color3f SpectralDiffuse::eval(const Vec3f & dirIn, const Vec3f &scattered, const HitInfo & hit) const 
{
	return albedo->value(hit) * max(0.0f, dot(scattered, hit.sn) / M_PI);
}

float SpectralDiffuse::pdf(const Vec3f & dirIn, const Vec3f &scattered, const HitInfo & hit) const 
{
	return max(0.0f, dot(scattered, hit.sn) / M_PI);
}